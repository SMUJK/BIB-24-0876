...
for (m in 1:14) {
  ...
  all_chr_results<-foreach(chromsome = 1:22) %dopar% {
    ...
    wd2<-paste(c("/gwas/gwas_ch",chromsome,".csv"),collapse="")
    ...
    wd3<-paste(c("/lookup/lookup_ch",chromsome,".csv"),collapse="")
    lookup.ch<-read.csv(wd3,header = T)
    qtl_files<-list.files(paste0("/sqtl/",tissue))
    wd1<-grepl(paste0("chr",chromsome,".par"),qtl_files) %>% qtl_files[.]
    eqtl.ch<-read_parquet(paste0("/sqtl/",tissue,"/",wd1))
